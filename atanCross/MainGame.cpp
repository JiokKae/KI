...
HRESULT MainGame::Init(HWND hWnd, HINSTANCE hInst)
{
	...
	circle[0].SetPos({ 300,300 });
	circle[1].SetPos({ 400,500 });
...
void MainGame::Render(HDC hdc) 
{
	...
	if (circle)
	{
		for (int i = 0; i < 2; i++)
		{
			circle[i].Render(hdcMem);
		}
	}

	MoveToEx(hdcMem, circle[0].GetPos().x, circle[0].GetPos().y, NULL);
	LineTo(hdcMem, circle[1].GetPos().x, circle[1].GetPos().y);

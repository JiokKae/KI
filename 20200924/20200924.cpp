...
/*
	전역변수 사용시 주의할 점
	1. 메모리가 항상 상주해 있다.
	2. 어디서나 접근이 가능하기 때문에 코드 관리가 안되는 측면이 있다.
*/

/*
	스택 메모리 : 일정한 코드영역 {} 안에서 선언되고 데이터 수정이 가능
	( 코드 영역이 종료될 때 메모리 자동 해제 )
	힙 메모리 : new 키워드를 사용하는 동적할당
	( delete 키워드를 사용해서 직접 해제 )
	데이터 메모리 : 일정한 코드영역 {} 밖에서 선언되고 데이터 수정이 가능
	( 프로그램이 종료될 때 자동 해제 )
